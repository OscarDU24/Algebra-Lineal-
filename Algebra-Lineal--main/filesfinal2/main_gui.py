...
class AppCalculadora(ctk.CTk):
    ...
    def generar_cuadricula_matriz(self):
        ...
        # --- NUEVA ADVERTENCIA DE RENDIMIENTO GRÁFICO ---
        if m > 12 or n > 12:
            respuesta = messagebox.askyesno(
                "Advertencia de Rendimiento",
                f"Estás a punto de generar {m * (n + 1)} casillas de texto.\n\n"
                "Cargar tantos elementos gráficos al mismo tiempo exige mucho al procesador y podría poner la pantalla lenta.\n\n"
    ...
    def _matriz_a_string(self, matriz, formato):
        ...
        for i in matriz:
            ...
            for j in range(len(matriz[0]) - 1):
                if formato == 'fr':
                    str_fila.append(f"{conv.convertir_a_fraccion(i[j])}")
                else:
                    str_fila.append(f"{i[j]:0.2f}".rstrip("0").rstrip("."))
            if formato == 'fr':
                str_fila.append(f"{conv.convertir_a_fraccion(i[-1])}")
            else:
                str_fila.append(f"{i[-1]:0.2f}".rstrip("0").rstrip("."))   
    ...
    def accion_resolver(self):
        """Orquesta el cálculo delegándolo a un hilo secundario."""
        try:
        ...
        # --- NUEVA ADVERTENCIA DE RENDIMIENTO MATEMÁTICO ---
        if m > 12 or n > 12:
            respuesta = messagebox.askyesno(
                "Aviso de Procesamiento Pesado",
                "El sistema es muy grande. El registro del paso a paso generará miles de líneas de texto.\n\n"
                "El cálculo se hará en segundo plano, por lo que la ventana NO se congelará, pero puede tardar unos segundos en mostrar el resultado.\n\n"
                "¿Deseas iniciar el cálculo?"
            )
            if not respuesta:
                return
        ...
        self.btn_resolver.configure(state="disabled", text="Calculando...", fg_color="#555555")
        self._escribir_en_visor("Procesando cálculos masivos... Por favor espera, la ventana sigue activa.")

        # Iniciar el hilo secundario
        hilo = threading.Thread(
            target=self._tarea_matematica,
            args=(matriz_original, modo, metodo_gui, formato, n)
        )
        hilo.start()

    def _tarea_matematica(self, matriz_original, modo, metodo_gui, formato, n):
        """Todo el procesamiento pesado ocurre aquí sin molestar a Tkinter."""
        ...
        # Regresar al hilo principal para actualizar la pantalla (Crucial)
        self.after(0, self._finalizar_calculo, texto_final)

    def _finalizar_calculo(self, texto_final):
        """Vuelve a habilitar la interfaz y muestra la respuesta."""
        self._escribir_en_visor(texto_final)
